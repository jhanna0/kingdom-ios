...
--
-- IMPORTANT: Gold = Meat × 0.6 (MEAT_MARKET_VALUE from hunting/config.py)
-- This is the ONLY way hunting creates gold in the economy.
...
SELECT 
    ...
    ROUND(SUM((session_data->>'total_meat')::int + (session_data->>'bonus_meat')::int) * 0.6) AS total_gold,
    ROUND(SUM(EXTRACT(EPOCH FROM (completed_at - started_at)) / 3600.0)::numeric, 2) AS total_hours,
    ...
    ROUND((SUM((session_data->>'total_meat')::int + (session_data->>'bonus_meat')::int) * 0.6 / 
           NULLIF(SUM(EXTRACT(EPOCH FROM (completed_at - started_at)) / 3600.0), 0))::numeric, 2) AS gold_per_hour
FROM hunt_sessions
...
SELECT 
    ...
    ROUND(SUM((h.session_data->>'total_meat')::int + (h.session_data->>'bonus_meat')::int) * 0.6) AS gold,
    ...
    ROUND((SUM((h.session_data->>'total_meat')::int + (h.session_data->>'bonus_meat')::int) * 0.6 / 
...
-- =============================================
-- HUNTING: Meat & Gold Breakdown by Animal
-- =============================================
SELECT 
    ...
    ROUND(SUM((h.session_data->>'total_meat')::int + (h.session_data->>'bonus_meat')::int) * 0.6) AS total_gold,
    ROUND(AVG((h.session_data->>'total_meat')::int + (h.session_data->>'bonus_meat')::int)::numeric, 2) AS avg_meat_per_hunt,
    ROUND(AVG((h.session_data->>'total_meat')::int + (h.session_data->>'bonus_meat')::int) * 0.6, 2) AS avg_gold_per_hunt,
...
-- =============================================
-- HUNTING: Top Drops (Last 24 Hours)
-- =============================================
SELECT 
    u.display_name,
    h.session_data->>'animal_id' AS animal,
    ...
    ROUND(((h.session_data->>'total_meat')::int + (h.session_data->>'bonus_meat')::int) * 0.6) AS gold,
    ROUND(EXTRACT(EPOCH FROM (h.completed_at - h.started_at))::numeric, 1) AS seconds,
    h.completed_at
FROM hunt_sessions h
JOIN users u ON u.id = h.created_by
WHERE h.status = 'completed'
...
-- =============================================
-- NOTE: Fishing gives meat but does NOT give gold directly.
-- Meat must be sold on the market or used as food for actions.
SELECT 
    u.display_name,
